...
app.get('/api/tokens', async (req, res) => {
    const { chainId, q } = req.query;
    if (!chainId || !q) {
        return res.status(400).json({ error: 'Missing chainId or q parameter' });
    }

    try {
        const response = await fetch(`https://api.1inch.dev/v1.2/${chainId}/search?q=${encodeURIComponent(q)}`, {
            headers: { Authorization: `Bearer ${process.env.REACT_APP_INCH_API_KEY}` },
        });
